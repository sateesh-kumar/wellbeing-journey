...
$pdo = getDBConnection();
$pageTitle = "Connection & Love — Results";
...
function getScoreInterpretation(int $score, int $max = 25): array {
    ...
    if ($pct >= 90) {
        return [
            'level' => 'Thriving',
            'color' => '#4CAF50',
            'description' => 'You are excelling across your connections and relationships.'
        ];
    }
    if ($pct >= 75) {
        return [
            'level' => 'Flourishing',
            'color' => '#8BC34A',
            'description' => 'Your connections are strong with a few areas to nurture.'
        ];
    }
    if ($pct >= 55) {
        return [
            'level' => 'Growing',
            'color' => '#C9A84C',
            'description' => 'You\'re making meaningful progress — keep building.'
        ];
    }
    if ($pct >= 35) {
        return [
            'level' => 'Developing',
            'color' => '#FF9800',
            'description' => 'Several areas need attention and intentional effort.'
        ];
    }
    return [
        'level' => 'Needs Work',
        'color' => '#F44336',
        'description' => 'This is a great time to focus on rebuilding your foundations.'
    ];
}
...
try {
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        $stmt = $pdo->prepare("SELECT cla.*,
               COALESCE(ts.total_score,
                   (COALESCE(cla.self_awareness_rating,0) + COALESCE(cla.emotional_connection_rating,0) +
                    COALESCE(cla.family_friends_rating,0) + COALESCE(cla.love_expression_rating,0) +
                    COALESCE(cla.community_rating,0))
               ) AS total_score
        FROM connection_love_assessments cla
        LEFT JOIN total_scores ts
            ON ts.user_id = cla.user_id
           AND ts.pillar_id = 1
           AND ts.recorded_at = (
               SELECT MAX(t2.recorded_at) FROM total_scores t2
               WHERE t2.user_id = cla.user_id AND t2.pillar_id = 1
               AND t2.recorded_at <= cla.assessment_date + INTERVAL 1 MINUTE
           )
        WHERE cla.id = ? AND cla.user_id = ?");
    ...
    $stmt = $pdo->prepare("
        SELECT id, assessment_date,
               (COALESCE(self_awareness_rating,0) + COALESCE(emotional_connection_rating,0) +
                COALESCE(family_friends_rating,0) + COALESCE(love_expression_rating,0) +
                COALESCE(community_rating,0)) AS total_score
        FROM connection_love_assessments 
        WHERE user_id = ? 
    ...
    error_log("Database error in view_results_connection.php: " . $e->getMessage());
...
<body>
    <div class="container">
        ...
        <header class="header">
            <h1>Connection &amp; Love <span class="accent">Results</span></h1>
            <p class="subtitle">Track your wellbeing journey across relationships, love, and community.</p>
            <a href="connection_and_love.php" class="btn btn-primary" style="margin-top:0.75rem;display:inline-block;">+ New Assessment</a>
        </header>
        ...
        <div class="results-layout">
            ...
            <aside class="history-sidebar" role="complementary" aria-label="Assessment History">
                ...
                <?php if (empty($allAssessments)): ?>
                    <p class="empty-state">No assessments yet.<br><a href="connection_and_love.php">Take your first one →</a></p>
                <?php else: ?>
            ...
            <main class="results-main" role="main">
                ...
                <?php else: ?>
                    <div class="empty-results">
                        ...
                        <p>Select an entry from the history panel, or <a href="connection_and_love.php">take a new assessment</a>.</p>
                    </div>
                <?php endif; ?>
            </main>
