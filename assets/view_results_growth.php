...
$pdo = getDBConnection();
$pageTitle = "Growth & Learning — Results";
...
function getScoreInterpretation(int $score, int $max = 25): array {
    ...
    if ($pct >= 90) {
        return [
            'level' => 'Thriving',
            'color' => '#4CAF50',
            'description' => 'You are excelling across your learning and growth journey.'
        ];
    }
    if ($pct >= 75) {
        return [
            'level' => 'Flourishing',
            'color' => '#8BC34A',
            'description' => 'Your growth mindset is strong with a few areas to develop.'
        ];
    }
    if ($pct >= 55) {
        return [
            'level' => 'Growing',
            'color' => '#C9A84C',
            'description' => 'You\'re making meaningful progress in your learning habits.'
        ];
    }
    if ($pct >= 35) {
        return [
            'level' => 'Developing',
            'color' => '#FF9800',
            'description' => 'Several learning areas need attention and intentional effort.'
        ];
    }
    return [
        'level' => 'Needs Work',
        'color' => '#F44336',
        'description' => 'This is a great time to focus on rebuilding your growth foundations.'
    ];
}
...
try {
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        $stmt = $pdo->prepare("SELECT gla.*,
               COALESCE(ts.total_score,
                   (COALESCE(gla.joy_rating,0) + COALESCE(gla.engagement_rating,0) +
                    COALESCE(gla.growth_rating,0) + COALESCE(gla.purpose_rating,0) +
                    COALESCE(gla.achievement_rating,0))
               ) AS total_score
        FROM growth_learning_assessments gla
        LEFT JOIN total_scores ts
            ON ts.user_id = gla.user_id
           AND ts.pillar_id = 2
           AND ts.recorded_at = (
               SELECT MAX(t2.recorded_at) FROM total_scores t2
               WHERE t2.user_id = gla.user_id AND t2.pillar_id = 2
               AND t2.recorded_at <= gla.assessment_date + INTERVAL 1 MINUTE
           )
        WHERE gla.id = ? AND gla.user_id = ?");
    ...
    $stmt = $pdo->prepare("
        SELECT id, assessment_date,
               (COALESCE(joy_rating,0) + COALESCE(engagement_rating,0) +
                COALESCE(growth_rating,0) + COALESCE(purpose_rating,0) +
                COALESCE(achievement_rating,0)) AS total_score
        FROM growth_learning_assessments
        WHERE user_id = ?
    ...
    error_log("Database error in view_results_growth.php: " . $e->getMessage());
    $errorMessage = "An error occurred while loading your assessments.";
}

$categories = [
    ['name' => 'Joy & Gratitude',              'field' => 'joy'],
    ['name' => 'Engagement & Flow',            'field' => 'engagement'],
    ['name' => 'Strengths & Personal Growth',  'field' => 'growth'],
    ['name' => 'Purpose & Contribution',       'field' => 'purpose'],
    ['name' => 'Achievement & Resilience',     'field' => 'achievement'],
];
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="container">
        ...
        <header class="header">
            <h1>Growth &amp; Learning <span class="accent">Results</span></h1>
            <p class="subtitle">Track your learning mindset, habits, and growth over time.</p>
            <a href="growth_and_learning.php" class="btn btn-primary" style="margin-top:0.75rem;display:inline-block;">+ New Assessment</a>
        </header>
        ...
        <div class="results-layout">
            ...
            <aside class="history-sidebar" role="complementary" aria-label="Assessment History">
                ...
                <?php if (empty($allAssessments)): ?>
                    <p class="empty-state">No assessments yet.<br><a href="growth_and_learning.php">Take your first one →</a></p>
                <?php else: ?>
            ...
            <main class="results-main" role="main">
                ...
                <?php else: ?>
                    <div class="empty-results">
                        ...
                        <p>Select an entry from the history panel, or <a href="growth_and_learning.php">take a new assessment</a>.</p>
                    </div>
                <?php endif; ?>
            </main>
